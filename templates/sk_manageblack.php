					...
					<?php if($comment->pc!=$first) { $first=$comment->pc; ?><th scope="row" class="check-column"><input type='checkbox' name='checked_pcs[]' value='<?php echo $comment->block_id; ?>' /></th>
						...
						<td class="manage-column column-author"><?if($comment->id!=NULL) { ?><div id="submitted-on"><?php
								...
								if($comment->email!="")
									...
									_e('No email registry', 'sk'); ?></div><div><p><img src='../wp-content/plugins/schreikasten/img/<?php $img='ham.png'; if($comment->status==SK_SPAM) $img='spam.png'; if($comment->status==SK_BLACK) $img='black.png'; if($comment->status==SK_MOOT) $img='moot.png'; echo $img; ?>'> <?php echo $comment->text; ?></p><?php
							$act_message="";
